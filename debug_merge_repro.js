...
const sandbox = {
  ...
  state: {
    vocabularyStudy: {
      ...
      entries: [{
        id: 'apple', word: 'apple', partOfSpeech: '名詞',
        ...
        lastJudgedAt: 1000, createdAt: 1000
      }],
      progressMap: {},
    ...
    vocabularyTodayHistoryMap: {
      '2026-08-25': { 'apple|名詞': { word: 'apple', partOfSpeech: '名詞', pronunciation: '○', meaning: '○', lastJudgedAt: 1000 } }
    },
...
vm.runInNewContext(snippet, sandbox, { filename: 'mobile-word-merge-regression.js' });
...
console.log('study ids after sync', sandbox.state.vocabularyStudy.entries.map((entry) => String(entry.id).trim()).sort());

sandbox.handleVocabularyTodayHistorySyncRemoteSnapshot({
  ok: true,
  exists: true,
  uid: 'uid-1',
  historyMap: {
...
console.log('history keys after history sync', Object.keys(sandbox.state.vocabularyTodayHistoryMap['2026-08-25'] || {}));
console.log('study ids final', sandbox.state.vocabularyStudy.entries.map((entry) => String(entry.id).trim()).sort());
