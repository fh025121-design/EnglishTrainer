...
const sandbox = {
  ...
  buildVocabularyRealStudyState:()=>({ targetWordCount:1000, entries:[], progressMap:{}, session:{ questionCount:0, failedWordIds:[], recentFailedWordIds:[] }, gradeSummary:{5:{ total:0, mastered:0 }, 4:{ total:0, mastered:0 }, 3:{ total:0, mastered:0 }} }),
...
sandbox.state.wordLearningState = map;
sandbox.state.vocabularyStudy = { entries:[ { id:'legacy-1', word:'legacy', pronunciation:{level:5}, meaningState:{level:5}, meaning:'旧語彙', partOfSpeech:'名詞' }, { id:'legacy-2', word:'legacy-2', pronunciation:{level:5}, meaningState:{level:5}, meaning:'旧語彙2', partOfSpeech:'名詞' } ] };
const sanitized = sandbox.window.sanitizeWordLearningStateMap(map);
console.log('sanitized', sanitized);
console.log('display statuses', Object.values(sanitized).map((entry) => ({ wordId: entry.wordId, status: sandbox.window.getWordLearningStateDisplayStatus(entry), questionCount: entry.questionCount })));
console.log('entries', sandbox.window.getWordLearningStateProgressEntries('learning'));
