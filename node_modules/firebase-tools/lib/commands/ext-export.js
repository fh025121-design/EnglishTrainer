...
exports.command = new command_1.Command("ext:export")
    ...
    .option(`-e`, `--extension-instance <instanceId>`, `scope the export to the single instance with the specified instance id`)
...
async function extHandler(options) {
    ...
    if (options.extensionInstance) {
        ...
        if (have.length === 0) {
            logger_1.logger.info(`No extension instances installed on ${projectId} match specified instance ID ${options.extensionInstance}.`);
            return;
        }
    }
...
async function fnHandler(options) {
    ...
    const projectId = (0, projectUtils_1.needProjectId)(options);
    const instance = await (0, extensionsApi_1.getInstance)(projectId, options.extensionInstance);
    if (typeof instance === "undefined") {
        logger_1.logger.info(`No extension matching instance ID ${options.extensionInstance} found`);
        return;
    }
    if (instance.state !== "ACTIVE" && !options.force) {
        throw new error_1.FirebaseError(`Extension ${options.extensionInstance} is in state ${instance.state}. To export a non-ACTIVE extension, use the --force option.`);
    }
    ...
    if ((0, env_1.hasUserEnvs)(writeLocation)) {
        ...
        (0, fs_1.mkdirSync)((0, path_1.join)(writeLocation.projectDir, writeLocation.configDir ?? instanceId), {
            recursive: true,
        });
...
function kitExportTarget(instanceId, projectId, options) {
    ...
    if (typeof options.projectRoot !== "undefined") {
        return {
            functionsSource: instanceId,
            configDir: (0, path_1.join)(options.projectRoot, instanceId),
            projectId: projectId,
            isEmulator: false,
