(function () {
  ...
  const MOBILE_VOCABULARY_INACTIVITY_MS = 60000;
  let mobileVocabularyLastUserActivityAt = 0;
  let mobileVocabularyReactivationSyncPending = false;
  ...
  function mergeWordLearningStateByLatest(baseMap, incomingMap) {
    ...
    allIds.forEach((wordId) => {
      ...
      if (!incomingEntry && baseEntry) {
        merged[wordId] = { ...baseEntry, wordId };
        return;
      }
      if (!baseEntry || !incomingEntry) {
        return;
      ...
      const baseLastStudiedAt = Number(baseEntry.lastStudiedAt) || 0;
      const incomingLastStudiedAt = Number(incomingEntry.lastStudiedAt) || 0;
      const finalQuestionCount = Math.max(baseQuestionCount, incomingQuestionCount);
      const hasLearnedBase = baseQuestionCount > 0;
      const hasLearnedIncoming = incomingQuestionCount > 0;

      let currentWinner = baseEntry;
      let finalLastStudiedAt = Math.max(baseLastStudiedAt, incomingLastStudiedAt);

      if (hasLearnedBase && !hasLearnedIncoming) {
        currentWinner = baseEntry;
        finalLastStudiedAt = baseLastStudiedAt;
      } else if (!hasLearnedBase && hasLearnedIncoming) {
        currentWinner = incomingEntry;
        finalLastStudiedAt = incomingLastStudiedAt;
      } else if (incomingLastStudiedAt >= baseLastStudiedAt) {
        currentWinner = incomingEntry;
        finalLastStudiedAt = Math.max(baseLastStudiedAt, incomingLastStudiedAt);
      } else {
        currentWinner = baseEntry;
        finalLastStudiedAt = Math.max(baseLastStudiedAt, incomingLastStudiedAt);
      }
      ...
      const mergedEntry = {
        wordId,
        pronunciationStatus: normalizeWordLearningStatus(currentWinner.pronunciationStatus || "－", "－"),
        meaningStatus: normalizeWordLearningStatus(currentWinner.meaningStatus || "－", "－"),
        lastSelfResult: String(currentWinner.lastSelfResult || "").trim() || null,
        lastStudiedAt: finalLastStudiedAt,
        questionCount: finalQuestionCount,
        perfectPairCount: Math.max(0, Number(currentWinner.perfectPairCount) || 0),
        isMastered: Boolean(currentWinner.isMastered),
        learningStateStatus: String(currentWinner.learningStateStatus || getWordLearningStateStatusFromEntry(currentWinner) || "unlearned").trim() || "unlearned"
      };

      if (finalQuestionCount <= 0) {
        mergedEntry.pronunciationStatus = "－";
        mergedEntry.meaningStatus = "－";
        mergedEntry.lastSelfResult = null;
        mergedEntry.lastStudiedAt = 0;
        mergedEntry.perfectPairCount = 0;
        mergedEntry.isMastered = false;
        mergedEntry.learningStateStatus = "unlearned";
      }

      merged[wordId] = mergedEntry;
    });

    return sanitizeWordLearningStateMap(merged);
  }
  ...
  function handleVocabularyTodayHistorySyncRemoteSnapshot(snapshot) {
    ...
    if (!snapshot?.ok || !snapshot.exists || !uid) {
      return;
    }
    ...
    const hasIncomingEntries = Boolean(incomingMap) && Object.keys(incomingMap).length > 0;
    ...
    if (!incomingMap || !hasIncomingEntries) {
      if (hasLocalEntries && (!remoteUpdatedAtMs || remoteUpdatedAtMs <= localCompareUpdatedAtMs)) {
    ...
    if (sameUidCanonical) {
      ...
      state.vocabularyTodayHistoryMap = normalizeVocabularyTodayHistoryMap(
        mergeVocabularyTodayHistoryMapByLatest(localBaseline, state.vocabularyTodayHistoryMap)
      );
      saveVocabularyTodayHistoryMap();
      if (uid) {
        const mobileSyncKey = getMobileVocabularyTodayHistoryStorageKey(uid);
    ...
    const shouldKeepLocalForStaleRemote = Boolean(remoteUpdatedAtMs > 0 && localCompareUpdatedAtMs > remoteUpdatedAtMs)
      || (!Object.keys(incomingMap || {}).length && hasLocalEntries && (!remoteUpdatedAtMs || remoteUpdatedAtMs <= localCompareUpdatedAtMs));
    const mergedHistoryMap = shouldKeepLocalForStaleRemote
      ? normalizeVocabularyTodayHistoryMap(localBaseline) || {}
      : mergeVocabularyTodayHistoryMapByLatest(localBaseline, incomingMap);
  ...
  function getVocabularyTeacherCheckSummaryCounts() {
    const map = sanitizeWordLearningStateMap(state.wordLearningState || {});
    const counts = { unconfirmed: 0, delta: 0, checked: 0 };
    ...
    Object.values(map)
      ...
      .forEach((entry) => {
        ...
        const hasUnconfirmed = (pronunciationStatus === "○" || meaningStatus === "○") && !hasDelta && !isChecked;
  ...
  function getVocabularyTeacherCheckCandidates(mode = "auto") {
    ...
    const entries = Object.values(map)
      ...
      .map((entry) => {
        ...
        const isChecked = pronunciationStatus === "◎" && meaningStatus === "◎";
        const hasUnconfirmed = (pronunciationStatus === "○" || meaningStatus === "○") && !hasDelta && !isChecked;
        const bucket = hasDelta ? "delta" : isChecked ? "checked" : hasUnconfirmed ? "unconfirmed" : null;
    ...
    if (modeKey === "unconfirmed") {
      candidates = entries.filter((entry) => entry.bucket === "unconfirmed");
    } else if (modeKey === "checked") {
      candidates = entries.filter((entry) => entry.bucket === "checked");
    } else if (modeKey === "delta") {
      candidates = entries.filter((entry) => entry.bucket === "delta");
    } else if (modeKey === "all") {
      candidates = entries;
    } else {
      const modeOrder = ["unconfirmed", "delta", "checked"];
      for (const bucketName of modeOrder) {
        const bucketCandidates = entries.filter((entry) => entry.bucket === bucketName);
  ...
  function getVocabularyTeacherCheckPageInfo(session) {
    ...
    const pageSize = 1;
    const pageIndex = Math.max(0, Number(session?.pageIndex || 0) || 0);
  ...
  function persistVocabularyTeacherCheckDecision(wordId, decision = {}) {
    const normalizedId = String(wordId || "").trim();
    if (!normalizedId) return null;
    const now = Date.now();
    const map = sanitizeWordLearningStateMap(state.wordLearningState || {});
    ...
    const nextEntry = {
      ...
      learningStateStatus: String(currentEntry.learningStateStatus || getWordLearningStateStatusFromEntry(currentEntry) || "learning").trim() || "learning",
      isMastered: Boolean(currentEntry.isMastered)
    };
  ...
  function finalizeVocabularyTeacherCheckSession() {
    ...
    Object.entries(session.decisions || {}).forEach(([wordId, decision]) => {
      ...
      const normalizedId = String(wordId || "").trim();
      if (!normalizedId) return;
      persistVocabularyTeacherCheckDecision(normalizedId, decision);
    });
  ...
  function completeVocabularyTeacherCheckCurrentBlock() {
    ...
    persistVocabularyTeacherCheckDecision(currentCandidate.id, decision);
  ...
  function finalizeCurrentTeacherCheckItemAndReturnHome() {
    if (!state.teacherCheckSession || state.teacherCheckSession.phase !== "active") {
      renderSpeakingHome();
      return;
    }
    const session = state.teacherCheckSession;
  ...
  function renderVocabularyTeacherCheckSetupScreen() {
    ...
    const summary = getVocabularyTeacherCheckSummaryCounts();
    const modeLabels = {
      auto: "おまかせ",
      unconfirmed: "未確認の○",
      delta: "△あり",
    ...
    metaEl.textContent = "先生チェック";

    contentEl.innerHTML = `
      <div class="vocabulary-teacher-check-setup">
        <div class="vocabulary-teacher-check-setup-group">
          <div class="vocabulary-teacher-check-setup-label">先生チェック</div>
          <button type="button" class="secondary-btn ${selectedMode === "auto" ? "is-selected" : ""}" data-teacher-check-action="mode" data-teacher-check-mode="auto">[ おまかせ ]</button>
        </div>
        <div class="vocabulary-teacher-check-setup-group">
          <div class="vocabulary-teacher-check-setup-label">または指定</div>
          ${["unconfirmed", "delta", "checked"].map((modeValue) => `
            <button type="button" class="secondary-btn ${selectedMode === modeValue ? "is-selected" : ""}" data-teacher-check-action="mode" data-teacher-check-mode="${modeValue}">[ ${modeLabels[modeValue]} ${summary[modeValue] || 0}語 ]</button>
          `).join("")}
        </div>
      </div>
    `;
  ...
  function renderVocabularyTeacherCheckScreen() {
    ...
    const bothSelected = String(decision.pronunciation || "none").trim() !== "none" && String(decision.meaning || "none").trim() !== "none";
    ...
    if (nextBtn) {
      nextBtn.classList.toggle("hidden", !bothSelected);
      nextBtn.textContent = "次の1語";
    }
    if (completeBtn) {
      completeBtn.classList.toggle("hidden", !bothSelected);
      completeBtn.textContent = "完了してホームへ";
    }
  ...
  window.getVocabularyTeacherCheckSummaryCounts = getVocabularyTeacherCheckSummaryCounts;
  ...
  window.getVocabularyTeacherCheckPageInfo = getVocabularyTeacherCheckPageInfo;
  window.persistVocabularyTeacherCheckDecision = persistVocabularyTeacherCheckDecision;
  window.finalizeVocabularyTeacherCheckSession = finalizeVocabularyTeacherCheckSession;
  window.completeVocabularyTeacherCheckCurrentBlock = completeVocabularyTeacherCheckCurrentBlock;
  ...
  async function initializeWordLearningStateSyncForCurrentUser(options = {}) {
    ...
    if (typeof remoteLoad === "function") {
      try {
        const remoteResult = await remoteLoad({ targetUid: uid });
        if (remoteResult?.ok && remoteResult.wordLearningState && typeof remoteResult.wordLearningState === "object") {
          remoteMap = sanitizeWordLearningStateMap(remoteResult.wordLearningState || {});
        }
      } catch (_error) {
        remoteMap = {};
      }
    ...
    const mergedMap = mergeWordLearningStateByLatest(localMap, remoteMap);
    const finalMap = Object.keys(mergedMap || {}).length ? sanitizeWordLearningStateMap(mergedMap || {}) : buildWordLearningStateMap();

    state.wordLearningState = finalMap;
    saveWordLearningStateForSync(finalMap, uid);

    if (typeof window.saveMobileWordLearningStateToFirestore === "function") {
      try {
        await window.saveMobileWordLearningStateToFirestore(finalMap, {
          targetUid: uid,
          allowCreate: true,
          sourceDeviceId: String(getMobileBrowserDeviceId() || "").trim(),
          sourceDeviceName: sanitizeMobileLearningHistoryDeviceName(getMobileLearningHistoryDeviceName())
        ...
        // Ignore remote-save failures in the initial bootstrap path; the merged local state is already preserved.
      }
    }

    return Object.keys(finalMap || {}).length > 0;
  }
  ...
  function saveState(changedWordId = "") {
    ...
    const hasStudyState = Boolean(state.vocabularyStudy && sanitizeVocabularyStudyState(state.vocabularyStudy));
    ...
    if (uid && vocabularyStateOwnerUid === uid && isVocabularyStateOwnerCurrentUid(uid) && safeStudyState) {
      scheduleMobileVocabularySync(changedWordId);
    }
  }

  function isMobileVocabularyBrowserActive() {
    const currentUid = String(getCurrentMobileFirebaseUser()?.uid || "").trim();
    if (!currentUid) return false;
    if (!mobileVocabularyLastUserActivityAt) return false;
    return Date.now() - mobileVocabularyLastUserActivityAt < MOBILE_VOCABULARY_INACTIVITY_MS;
  }

  function isMobileVocabularySyncWriteAllowed(uid = getCurrentMobileFirebaseUser()?.uid || "") {
    const currentUid = String(uid || getCurrentMobileFirebaseUser()?.uid || "").trim();
    if (!currentUid) return false;
    if (mobileVocabularyReactivationSyncPending) return false;
    if (!mobileVocabularyLastUserActivityAt) return false;
    return Date.now() - mobileVocabularyLastUserActivityAt < MOBILE_VOCABULARY_INACTIVITY_MS;
  }

  function markMobileVocabularyUserActivity() {
    const uid = String(getCurrentMobileFirebaseUser()?.uid || "").trim();
    const now = Date.now();
    const wasInactive = !isMobileVocabularyBrowserActive();
    mobileVocabularyLastUserActivityAt = now;
    if (!uid) return;
    if (!wasInactive) return;
    mobileVocabularyReactivationSyncPending = true;
    initializeMobileVocabularySyncForCurrentUser({ force: true }).catch(() => false).finally(() => {
      mobileVocabularyReactivationSyncPending = false;
    });
    initializeMobileVocabularyTodayHistorySyncForCurrentUser({ force: true }).catch(() => false).finally(() => {
      mobileVocabularyReactivationSyncPending = false;
    });
  }
  ...
  function isCurrentMobileChildUid(uid = getCurrentMobileFirebaseUser()?.uid || "") {
    ...
    if (currentUid === mobileCachedSonUid) {
      return true;
    }
    if (!mobileCachedSonUid) {
      mobileCachedSonUid = readMobileCachedSonUid();
  ...
  function versionIsBehind(currentVersion, incomingVersion) {
    const current = Math.max(0, Number(currentVersion) || 0);
    const incoming = Math.max(0, Number(incomingVersion) || 0);
    return incoming > current;
  }

  function isChildUidResetTarget(targetUid = "") {
    const safeTargetUid = String(targetUid || "").trim();
    if (!safeTargetUid) return false;
    if (targetUid === MOBILE_FIXED_CHILD_UID) return true;
    if (safeTargetUid === MOBILE_FIXED_CHILD_UID) return true;
    if (safeTargetUid === String(readMobileCachedSonUid() || "").trim()) return true;
    return isCurrentMobileChildUid(safeTargetUid);
  }

  function getMobileVocabularyResetAppliedStorageKey(uid = "") {
    const safeUid = String(uid || "").trim();
    return safeUid
      ? `english-trainer-mobile-vocabulary-reset-applied-v1:${safeUid}`
      : "english-trainer-mobile-vocabulary-reset-applied-v1";
  }

  function readMobileVocabularyResetAppliedState(uid = "") {
    const safeUid = String(uid || "").trim();
    if (!safeUid) return { resetVersion: 0, resetAtMs: 0 };
    try {
      const raw = window.localStorage.getItem(getMobileVocabularyResetAppliedStorageKey(safeUid));
      if (!raw) return { resetVersion: 0, resetAtMs: 0 };
      const parsed = JSON.parse(raw);
  ...
  function saveMobileVocabularyResetAppliedState(targetUid, resetVersion, resetAtMs) {
    const safeUid = String(targetUid || "").trim();
    if (!safeUid) return false;
    const payload = {
      resetVersion: Math.max(0, Number(resetVersion) || 0),
      resetAtMs: Math.max(0, Number(resetAtMs) || 0)
    };
    try {
      window.localStorage.setItem(getMobileVocabularyResetAppliedStorageKey(safeUid), JSON.stringify(payload));
      return true;
    } catch (_error) {
      return false;
    }
  ...
  function localHasStaleVocabularyState(targetUid = "", remoteResetVersion = 0, remoteResetAtMs = 0) {
    const safeTargetUid = String(targetUid || "").trim();
    if (!safeTargetUid || !isChildUidResetTarget(safeTargetUid)) {
      return false;
    }
    const localResetState = readMobileVocabularyResetAppliedState(safeTargetUid);
    const resetVersion = Math.max(0, Number(remoteResetVersion) || 0);
    const resetAtMs = Math.max(0, Number(remoteResetAtMs) || 0);
    const localResetVersion = Math.max(0, Number(localResetState?.resetVersion) || 0);
    const localResetAtMs = Math.max(0, Number(localResetState?.resetAtMs) || 0);
    if (remoteResetVersion > localResetVersion) {
      return true;
    }
    if (versionIsBehind(localResetVersion, resetVersion) || (resetVersion === localResetVersion && resetAtMs > localResetAtMs)) {
      return true;
    }
    const localStudyState = loadMobileVocabularyStateForSync(safeTargetUid);
    const localHistory = readVocabularyTodayHistoryMapFromStorageKey(getMobileVocabularyTodayHistoryStorageKey(safeTargetUid));
    return Boolean(
      (localStudyState && Array.isArray(localStudyState.entries) && localStudyState.entries.length > 0)
      || (localHistory && Object.keys(localHistory).length > 0)
    );
  }

  function applyMobileVocabularyChildResetIfNeeded(targetUid = "", remoteResetVersion = 0, remoteResetAtMs = 0) {
    const safeTargetUid = String(targetUid || "").trim();
    if (!targetUid || !isChildUidResetTarget(targetUid)) {
      return false;
    }
    if (safeTargetUid === MOBILE_FIXED_CHILD_UID) {
      // Fixed child UID reset must ignore stale cached family UID and stale local vocabulary state.
    }
    const localResetState = readMobileVocabularyResetAppliedState(safeTargetUid);
    const nextResetVersion = Math.max(0, Number(remoteResetVersion) || 0);
    const nextResetAtMs = Math.max(0, Number(remoteResetAtMs) || 0);
    const localResetVersion = Math.max(0, Number(localResetState?.resetVersion) || 0);
    const localResetAtMs = Math.max(0, Number(localResetState?.resetAtMs) || 0);
    if (remoteResetVersion > localResetVersion) {
      // remote reset supersedes stale local state for the active child UID.
    }
    if (nextResetVersion <= localResetVersion && (nextResetVersion !== localResetVersion || nextResetAtMs <= localResetAtMs)) {
      return false;
    }
    const freshStudy = buildVocabularyRealStudyState();
    const resetVersion = nextResetVersion;
    const resetAtMs = nextResetAtMs;
    state.teacherCheckSession = null;
  ...
  function handleVocabularySyncRemoteSnapshot(snapshot) {
    ...
    const currentLocal = sanitizeVocabularyStudyState(state.vocabularyStudy) ? state.vocabularyStudy : buildVocabularyRealStudyState();
    const localLearnedCount = getVocabularyStudyLearnedCount(currentLocal);
    const remoteLearnedCount = getVocabularyStudyLearnedCount(incomingStudy);
    const localCompareUpdatedAtMs = getVocabularyStudyMostRecentUpdatedAt(currentLocal);
    const remoteUpdatedAtMs = Number(snapshot?.updatedAtMs || 0) || 0;
    const sameUidCanonical = isSameUidSyncCanonical(uid);

    const shouldKeepLocalForStaleRemote = Boolean(remoteUpdatedAtMs > 0 && localCompareUpdatedAtMs > remoteUpdatedAtMs)
      || (!incomingStudy || !Array.isArray(incomingStudy.entries) || !incomingStudy.entries.length)
      && sanitizeVocabularyStudyState(currentLocal) && localCompareUpdatedAtMs >= remoteUpdatedAtMs;
    if (shouldKeepLocalForStaleRemote) {
  ...
  async function initializeMobileVocabularySyncForCurrentUser(options = {}) {
    ...
    if (remoteResult?.ok && remoteResult.exists) {
      ...
      const hasLocalStudy = Boolean(sanitizeVocabularyStudyState(localBaseline));
      if (hasLocalStudy) {
      ...
      vocabularySyncCurrentUid = uid;
      vocabularySyncReady = true;
      vocabularySyncAllowCreate = !hasLocalStudy;
      mobileVocabularyReactivationSyncPending = false;
    }
  ...
  async function flushMobileVocabularySync(changedWordId = "") {
    ...
    vocabularySyncInFlight = (async () => {
      do {
        ...
        const sourceStudy = loadMobileVocabularyStateForSync(uid) || (isVocabularyStateOwnerCurrentUid(uid) ? state.vocabularyStudy : null);
        const normalizedSource = sourceStudy && sanitizeVocabularyStudyState(sourceStudy) ? sourceStudy : (state.vocabularyStudy && sanitizeVocabularyStudyState(state.vocabularyStudy) ? state.vocabularyStudy : null);
        if (!normalizedSource) {
          break;
        }
  ...
  function scheduleMobileVocabularySync(changedWordId = "") {
    ...
    if (!isMobileVocabularySyncWriteAllowed(uid)) return;
    if (isMobileVocabularySyncRateLimited("study") || isMobileVocabularySyncDuplicateBurst("study")) {
      return;
    }
    const latest = state.vocabularyStudy && sanitizeVocabularyStudyState(state.vocabularyStudy)
  ...
  function applyMobileAuthState(user) {
    ...
    mobileVocabularyLastUserActivityAt = 0;
    mobileVocabularyReactivationSyncPending = false;
  ...
  function bindEvents() {
    document.addEventListener("pointerdown", markMobileVocabularyUserActivity, { passive: true });
    document.addEventListener("touchstart", markMobileVocabularyUserActivity, { passive: true });
    document.addEventListener("keydown", markMobileVocabularyUserActivity, { passive: true });
    ...
    if (elements.vocabularyTeacherCheckCompleteBtn) {
      elements.vocabularyTeacherCheckCompleteBtn.addEventListener("click", () => {
        finalizeCurrentTeacherCheckItemAndReturnHome();
      });
    }
